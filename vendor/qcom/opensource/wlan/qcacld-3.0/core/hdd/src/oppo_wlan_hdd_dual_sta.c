...
#define PROCFS_DRIVER_OPPO_DIR "oppo_dualSta"
 ...
 */
static ssize_t hdd_driver_oppo_read(struct file *file, char __user *buf,
 ...
 */
static const struct file_operations oppo_dual_sta_fops = {
	read: hdd_driver_oppo_read
};
 ...
 * Return:  None
 */
static void hdd_driver_oppo_procfs_remove(void)
{
	remove_proc_entry(PROCFS_DRIVER_1x1IOT_NAME, proc_dir_driver);
	pr_debug("/proc/%s/%s removed\n", PROCFS_DRIVER_OPPO_DIR,
					  PROCFS_DRIVER_1x1IOT_NAME);
	remove_proc_entry(PROCFS_DRIVER_OPPO_DIR, NULL);
 ...
 */
int hdd_driver_oppo_init(void)
{
	int status;

	status = hdd_driver_oppo_procfs_init();
	if (status) {
 ...
 * Return: None
 */
void hdd_driver_oppo_deinit(void)
{
	hdd_driver_oppo_procfs_remove();
}
