...
/*Xinyu.Zhao@PSW.MM.Display.LCD.Stability,2020-03-24 Change reset enable sequence for LCD power on spec.*/
...
*/
	...
	"qcom,mdss-dsi-vgl-2v-command",
	"qcom,mdss-dsi-vgl-3v-command",
#endif /*VENDOR_EDIT*/
};
...
*/
	...
	"qcom,mdss-dsi-vgl-2v-command-state",
	"qcom,mdss-dsi-vgl-3v-command-state",
#endif /*VENDOR_EDIT*/
};
...
static int dsi_panel_parse_oppo_config(struct dsi_panel *panel)
{
	...
	panel->oppo_priv.is_osc_support = utils->read_bool(utils->data,
		"oppo,osc_support");
	DSI_INFO("osc support mode: %s", panel->oppo_priv.is_osc_support ? "true" : "false");
...
void oppo_gpio_parse(struct dsi_panel *panel)
{
	...
	if (rc) {
		DSI_WARN("oppo,avdd-output-pulse-num is not defined, rc=%d\n", rc);
...
__attribute__((weak)) int dsi_panel_update_gatevol(struct dsi_panel *panel)
{
	return 0;
}
#endif /* VENDOR_EDIT */
...
#else /* VENDOR_EDIT */
/*Sheng.Li@OPLUS_FEATURE_PANEL_POWER,2020-05-23 add for panel power config */
	else{
		rc = dsi_panel_update_gatevol(panel);
		if (!rc)
			panel->panel_initialized = true;
	}
#endif /* VENDOR_EDIT */
