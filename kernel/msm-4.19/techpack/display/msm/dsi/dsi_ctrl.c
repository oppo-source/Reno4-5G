...
#define DSI_CTRL_ERR(c, fmt, ...) \
	do { \
		DRM_DEV_ERROR(NULL, "[msm-dsi-error]: %s: "\
				fmt, c ? c->name : "inv", ##__VA_ARGS__); \
		mm_display_kevent_named(MM_KEY_RATELIMIT_1H, fmt, ##__VA_ARGS__); \
	} while(0)
#endif /* VENDOR_EDIT */
...
/*Mark.Yao@PSW.MM.Display.LCD.Stable,2019-12-28 dma_tx done but irq not triggered */
			...
			if (dsi_ctrl->irq_info.irq_stat_refcount[DSI_SINT_CMD_MODE_DMA_DONE] > 1) {
				...
				mm_display_kevent("dma_tx irq trigger fixup", MM_KEY_RATELIMIT_NONE, "irq status=%x", status);
			}
			mm_display_kevent("dma_tx irq trigger err", MM_KEY_RATELIMIT_1H, "irq status=%x", status);
#endif
		} else {
