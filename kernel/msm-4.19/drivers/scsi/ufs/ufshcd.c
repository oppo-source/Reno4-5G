...
{
#if defined(VENDOR_EDIT) && defined(CONFIG_SCSI_UFSHCD_CMD_LOGGING)
	int scsi_block_reqs_cnt_bf = 0;
	int scsi_block_reqs_cnt_af = 0;
	unsigned long ca;
	unsigned long cb;

	ca = (unsigned long)__builtin_return_address(0);
	cb = (unsigned long)__builtin_return_address(2);
	scsi_block_reqs_cnt_bf = atomic_read(&hba->scsi_block_reqs_cnt);
#endif
	if (atomic_dec_and_test(&hba->scsi_block_reqs_cnt))
		scsi_unblock_requests(hba->host);
#if defined(VENDOR_EDIT) && defined(CONFIG_SCSI_UFSHCD_CMD_LOGGING)
	scsi_block_reqs_cnt_af = atomic_read(&hba->scsi_block_reqs_cnt);
	trace_printk("scsi_block_reqs_cnt from %d to %d, CA: %pS, CB: %pS\n",
			 scsi_block_reqs_cnt_bf, scsi_block_reqs_cnt_af,ca,cb);
#endif
}
...
#define UFSHCD_MAX_CMD_LOGGING	400
...
static void ufshcd_ungate_work(struct work_struct *work)
{
	...
	trace_qcom_ufs_log("ufshcd_ungate_work start");
	...
	trace_qcom_ufs_log("ufshcd_enable_clocks start");
...
start:
...
#if defined(VENDOR_EDIT) && defined(CONFIG_SCSI_UFSHCD_CMD_LOGGING)
			trace_oppo_ufshcd_clk_gating(dev_name(hba->dev),
					hba->clk_gating.state,__func__,
	...
	case CLKS_OFF:
		...
		if(queue_work(hba->clk_gating.clk_gating_workq,
			...
			trace_qcom_ufs_log("ufshcd_hold: queue ungate work");
			ufshcd_scsi_block_requests(hba);
		}
	...
	case REQ_CLKS_ON:
		if (async) {
			rc = -EAGAIN;
			hba->clk_gating.active_reqs--;
			trace_qcom_ufs_log("ufshcd_hold: async");
#if defined(VENDOR_EDIT) && defined(CONFIG_SCSI_UFSHCD_CMD_LOGGING)
			trace_ufshcd_clkgate_active_reqs_change(hba->clk_gating.active_reqs,
					__func__);
#endif
...
#if defined(VENDOR_EDIT) && defined(CONFIG_SCSI_UFSHCD_CMD_LOGGING)
#undef ufshcd_set_link_hibern8
static void ufshcd_set_link_hibern8(struct ufs_hba *hba)
{
	(hba)->uic_link_state = UIC_LINK_HIBERN8_STATE;
	trace_ufshcd_set_link_hibern8(hba->uic_link_state,
			(unsigned long)__builtin_return_address(0),
			(unsigned long)__builtin_return_address(2));
}
#endif
...
#if defined(VENDOR_EDIT) && defined(CONFIG_SCSI_UFSHCD_CMD_LOGGING)
	trace_qcom_ufs_log("ufshcd_gate_work: enter h8");
#endif
...
static void ufshcd_exit_clk_gating(struct ufs_hba *hba)
{
	...
	trace_qcom_ufs_log("cancle ungate work");
