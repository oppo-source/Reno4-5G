...
static inline bool scsi_host_is_busy(struct Scsi_Host *shost)
{
	if (shost->can_queue > 0 &&
		...
		trace_scsi_host_is_busy("host_busy",atomic_read(&shost->host_busy));
		return true;
	}
	if (atomic_read(&shost->host_blocked) > 0){
		trace_scsi_host_is_busy("host_blocked",atomic_read(&shost->host_blocked));
		return true;
	}
	if (shost->host_self_blocked){
		trace_scsi_host_is_busy("host_self_blocked",shost->host_self_blocked);
		return true;
	}
	return false;
}
				   ...
				   struct scsi_device *sdev)
{
	...
	if (shost->can_queue > 0 && busy >= shost->can_queue)
		goto starved;
	if (shost->host_self_blocked){
		trace_scsi_host_queue_ready(shost->host_self_blocked);
		goto starved;
	}
...
void scsi_block_requests(struct Scsi_Host *shost)
{
	...
	trace_scsi_self_blocked_change(shost->host_self_blocked);
}
