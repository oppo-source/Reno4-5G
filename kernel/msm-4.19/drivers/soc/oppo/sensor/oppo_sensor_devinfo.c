...
static void sensor_dev_work(struct work_struct *work)
{
	...
	//wait adsp probe,update the ps_match id
	do{
		if(!g_chip->is_ps_initialed)
		{
			msleep(6000);
			pr_info("%s  match_id %d is_ps_initialed = %d\n", __func__,g_chip->ps_match_id,g_chip->is_ps_initialed);
		}else {
			break;
		}
	}while(--ps_wait_loop);
...
static int oppo_devinfo_probe(struct platform_device *pdev)
{
	...
	g_chip = chip;
	g_chip->is_als_initialed = false;
	g_chip->is_ps_initialed = false;
