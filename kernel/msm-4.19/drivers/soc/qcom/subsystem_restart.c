...
static char modem_crash_reason[MAX_REASON_LEN] = {0};
...
#ifdef VENDOR_EDIT
//LiuWei@NETWORK.RF 2020/05/06, Add for report modem crash uevent
void subsystem_set_modem_crash_reason(char *reason)
{
	memset(modem_crash_reason, 0, sizeof(modem_crash_reason));
	if ((reason != NULL) && (reason[0] != '\0')){
		snprintf(modem_crash_reason, sizeof(modem_crash_reason), "MODEM_REASON=%s", reason);
	}

	modem_crash_reason[MAX_REASON_LEN - 1] = '\0';
}
EXPORT_SYMBOL(subsystem_set_modem_crash_reason);

void subsystem_send_uevent(struct subsys_device *dev)
{
	int ret_val;
	...
	if(modem_crash_reason[0] == 0){
		snprintf(modem_crash_reason, sizeof(modem_crash_reason), "MODEM_REASON=unkown");
	}
	envp[1] = (char *)&modem_crash_reason;
	envp[2] = 0;

	if(dev){
		ret_val = kobject_uevent_env(&(dev->dev.kobj), KOBJ_CHANGE, envp);
		if(!ret_val){
