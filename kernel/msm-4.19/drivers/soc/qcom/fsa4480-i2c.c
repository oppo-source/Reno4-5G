...
static int fsa4480_usbc_analog_setup_switches(struct fsa4480_priv *fsa_priv)
{
	...
	#ifdef VENDOR_EDIT
	/*Jianfeng.Qiu@PSW.MM.AudioDriver.HeadsetDet.2831248, 2020/02/27,
	 *Add for fix headset remove issue
	 */
	switch (mode.intval) {
	case POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER:
	case POWER_SUPPLY_TYPEC_NONE:
		if (atomic_read(&(fsa_priv->usbc_mode)) != mode.intval) {
			dev_info(dev, "%s: usbc_mode %d, change to %d\n",
				__func__, fsa_priv->usbc_mode.counter, mode.intval);
			atomic_set(&(fsa_priv->usbc_mode), mode.intval);
		}
		break;
	default:
