...
static irqreturn_t qcom_ipcc_irq_fn(int irq, void *data)
{
	...
	for (;;) {
        ...
        #else
        if(atomic_read(&qrtr_first_msg) == 1){
            pr_info("qrtr_first_msg IRQ for client_id: %u; signal_id: %u; virq: %d\n",
                qcom_ipcc_get_client_id(packed_id),
                qcom_ipcc_get_signal_id(packed_id), virq);
        }
        #endif //VENDOR_EDIT
...
static void msm_ipcc_resume(void)
{
	...
	#ifdef VENDOR_EDIT
	//ruansong@PSW.NW.DATA, 2020/04/04.
	if (strncmp(name, GLINK_NATIVE_ADSP_IRQ_NAME_IPCC, strlen(GLINK_NATIVE_ADSP_IRQ_NAME_IPCC)) == 0) {
		nw_adsp_wakeup_times++;
	} else if(strncmp(name, GLINK_NATIVE_CDSP_IRQ_NAME_IPCC, strlen(GLINK_NATIVE_CDSP_IRQ_NAME_IPCC)) == 0) {
		nw_cdsp_wakeup_times++;
	} else if(strncmp(name, GLINK_NATIVE_SLPI_IRQ_NAME_IPCC, strlen(GLINK_NATIVE_SLPI_IRQ_NAME_IPCC)) == 0){
		nw_slpi_wakeup_times++;
	} else if(strncmp(name, GLINK_NATIVE_MODEM_IRQ_NAME, strlen(GLINK_NATIVE_MODEM_IRQ_NAME)) == 0){
