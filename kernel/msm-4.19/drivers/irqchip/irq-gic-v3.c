...
extern u64 nw_ipcc_wakeup_times;
...
static void gic_show_resume_irq(struct gic_chip_data *gic)
{
	...
	for (i = find_first_bit((unsigned long *)pending, gic->irq_nr);
		...
		if(name != NULL)
		{
			...
			if(strncmp(name, WLAN_DATA_IRQ_NAME, strlen(WLAN_DATA_IRQ_NAME)) == 0) {
				wakeup_source_count_wifi++;
				nw_wifi_wakeup_times++;
			} else if((strncmp(name, MODEM_IRQ_NAME, strlen(MODEM_IRQ_NAME)) == 0)
				 || (strncmp(name, MODEM_IPA_IRQ_NAME, strlen(MODEM_IPA_IRQ_NAME)) == 0)
				 || (strncmp(name, GLINK_NATIVE_MODEM_IRQ_NAME, strlen(GLINK_NATIVE_MODEM_IRQ_NAME)) == 0)) {
				wakeup_source_count_modem++;
				if(strncmp(name, MODEM_IPA_IRQ_NAME, strlen(MODEM_IPA_IRQ_NAME)) == 0) {
					modem_wakeup_src_count[MODEM_IPA_WS_INDEX]++;
				}
				else if(strncmp(name, MODEM_IRQ_NAME, strlen(MODEM_IRQ_NAME)) == 0) {
					modem_wakeup_src_count[MODEM_QMI_WS_INDEX]++;
				}
			} else if((strncmp(name, ADSP_IRQ_NAME, strlen(ADSP_IRQ_NAME)) == 0)
				 || (strncmp(name, GLINK_NATIVE_ADSP_IRQ_NAME, strlen(GLINK_NATIVE_ADSP_IRQ_NAME)) == 0)) {
				wakeup_source_count_adsp++;
				nw_adsp_wakeup_times++;
			} else if((strncmp(name, CDSP_IRQ_NAME, strlen(CDSP_IRQ_NAME)) == 0)
				 || (strncmp(name, GLINK_NATIVE_CDSP_IRQ_NAME, strlen(GLINK_NATIVE_CDSP_IRQ_NAME)) == 0)) {
				wakeup_source_count_cdsp++;
				nw_cdsp_wakeup_times++;
			} else if((strncmp(name, SLPI_IRQ_NAME, strlen(SLPI_IRQ_NAME)) == 0)
				 || (strncmp(name, GLINK_NATIVE_SLPI_IRQ_NAME, strlen(GLINK_NATIVE_SLPI_IRQ_NAME)) == 0)) {
