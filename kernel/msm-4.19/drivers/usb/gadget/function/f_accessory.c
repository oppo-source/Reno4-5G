...
static int acc_setup(void)
{
	...
	if (ret)
		goto err;
#ifdef VENDOR_EDIT
	/* YanGang@BSP.CHG.Basic, 2019/12/12, set value to the describer of aoa device*/
	strlcpy(dev->manufacturer,"OPPO",ACC_STRING_SIZE);
	strlcpy(dev->model,"acce",ACC_STRING_SIZE);
	strlcpy(dev->description,"Android3",ACC_STRING_SIZE);
	strlcpy(dev->version,"123456",ACC_STRING_SIZE);
	strlcpy(dev->uri,"Android4",ACC_STRING_SIZE);
	strlcpy(dev->serial,"0123456",ACC_STRING_SIZE);
#endif
	return 0;
