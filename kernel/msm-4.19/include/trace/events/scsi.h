...
TRACE_EVENT(scsi_dispatch_cmd_error,
	...
	TP_printk("host_self_blocked=%u channel=%u id=%u lun=%u data_sgl=%u prot_sgl=%u" \
...
TRACE_EVENT(scsi_host_queue_ready,

	TP_PROTO(unsigned host_self_blocked),

	TP_ARGS(host_self_blocked),

	TP_STRUCT__entry(
		__field( unsigned,		host_self_blocked)
	),

	TP_fast_assign(
		__entry->host_self_blocked		= host_self_blocked;
	),

	TP_printk("host_self_blocked=%d",
		  __entry->host_self_blocked)
);


TRACE_EVENT(scsi_host_is_busy,

	TP_PROTO(const char *type, unsigned count),
	...
	TP_printk("type: %s, count =%d",
		  __get_str(type),__entry->count)
);

TRACE_EVENT(scsi_self_blocked_change,
	...
	TP_printk("host_self_blocked change to %d",
		  __entry->host_self_blocked)
);
