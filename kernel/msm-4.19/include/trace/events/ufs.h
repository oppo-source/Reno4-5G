...
TRACE_EVENT(ufshcd_set_link_hibern8,

		TP_PROTO(int state, unsigned long ca1, unsigned long ca2),

		TP_ARGS(state,ca1,ca2),

		TP_STRUCT__entry(
			__field(int, state)
			__field(unsigned long, ca1)
			__field(unsigned long, ca2)
			),
		...
		TP_printk("uic link state: %s, caller1: %pS, caller2: %pS",
			__print_symbolic(__entry->state,
				{ UIC_LINK_OFF_STATE, "UIC_LINK_OFF_STATE" },
				{ UIC_LINK_ACTIVE_STATE, "UIC_LINK_ACTIVE_STATE" },
...
TRACE_EVENT(oppo_ufshcd_clk_gating,
		TP_PROTO(const char *dev_name, int state, const char *func, unsigned long ca1, unsigned long ca2),

		TP_ARGS(dev_name, state, func, ca1, ca2),
		...
		TP_printk("%s: state changed to %s,current: %s  Caller1: %pS, Caller2: %pS", __get_str(dev_name),
				__print_symbolic(__entry->state,
					{ CLKS_OFF, "CLKS_OFF" },
					{ CLKS_ON, "CLKS_ON" },
					{ REQ_CLKS_OFF, "REQ_CLKS_OFF" },
...
TRACE_EVENT(ufshcd_scsi_block_requests,

	TP_PROTO(int prev, int curr, unsigned long ca),

	TP_ARGS(prev, curr, ca),

	TP_STRUCT__entry(
		__field(int, prev)
		__field(int, curr)
		__field(unsigned long, ca)
	),
	...
	TP_printk("scsi_block_reqs_cnt from %d to %d, CA: %pS",
		__entry->prev, __entry->curr, (void *)__entry->ca)
);

TRACE_EVENT(ufshcd_scsi_unblock_requests,
